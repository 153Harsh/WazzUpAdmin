...
const ReplyPage = () => {
  ...
  const attachmentOptions = [
    ...
    { icon: <IoDocumentText size={24} color="purple" />, label: "Document", accept: ".pdf,.doc,.docx,.txt" },
  ];
...
const scrollToMessage = (messageId) => {
  ...
  const goPrev = () => {
    if (!matchedMessages.length) return;
    const prev = (currentMatch - 1 + matchedMessages.length) % matchedMessages.length;
...
useEffect(() => {
  ...
  const handleUpdateMsg = useCallback((data) => {
    if (fromNoRef.current && isMatchingPhone(data.From || data.from, fromNoRef.current)) {
...
useEffect(() => {
  ...
  const handleUpdateNewUser = useCallback(async (data) => {
    if (fromNoRef.current && isMatchingPhone(data.From || data.from, fromNoRef.current)) {
      const uniqueMessages = [];
      const seenIds = new Set();
      const sortedMessages = [...(data.Messages || [])].sort((a, b) => a.timestamp - b.timestamp);
    ...
    setQueryData((prev) => {
      ...
      if (index !== -1) {
        ...
        const existingLastId = existingItem.Messages?.[existingItem.Messages.length - 1]?.id;
        const newLastId = data.Messages?.[data.Messages.length - 1]?.id;
        const shouldUpdateMessages = existingLastId !== newLastId;
        const shouldUpdateAdminReadLast = existingItem.adminReadLast !== data.adminReadLast;
        ...
        if (shouldUpdateMessages || shouldUpdateAdminReadLast) {
          ...
          if (Array.isArray(data.Messages) && data.Messages.length > 0 && shouldUpdateMessages) {
  ...
  }, [dbType, handleUpdateMsg, handleUpdateNewUser, handleUpdateRead, handleReplyTextSocket]);
...
useEffect(() => {
  ...
  useEffect(() => { fromNoRef.current = fromNo; }, [fromNo]);
  ...
  useEffect(() => {
    const syncDbType = (event) => {
      if (dbSwitchTimeoutRef.current) {
        clearTimeout(dbSwitchTimeoutRef.current);
      }
        ...
        <div className="bg-white w-full rounded-lg p-2 text-sm border-2 text-black mx-1 mb-4">
          ...
          {components.header && components.header.format && mediaUrl && (
            <div className="mb-2">
              ...
              {components.header.format === "TEXT" && components.header.text && (
          ...
          {components.body?.text && (
            <div className="whitespace-pre-wrap break-words mb-1">
              {parseWhatsAppFormatting(renderWithVariables(components.body.text))}
            </div>
          )}
    ...
    if (page === 1 && newData.length === 0) {
      toast((t) => (
        <div>
          ...
          <p>No data available for {dbType === 'company' ? 'Client' : 'DigiLateral'} database.</p>
        </div>
      ), { duration: 3000 });
    }
...
// console.log(JSON.stringify(messages, null, 2));
 ...
 const handleSendMessage = useCallback(async () => {
  ...
  const filteredData = useMemo(() => {
    const lowerCaseSearchQuery = (searchQuery || searchQuery1)?.toLowerCase() || "";
    const now = Date.now();
  ...
  useEffect(() => {
    ...
    fetchDetails();
  }, [ReplyedMobNo, handleGetCaseNo, filteredData.length, effectiveUserId, dbType]);
...
const handleSendMedia = async () => {
  ...
  try {
    ...
    if (fileType === "application" || mimeType.includes("pdf") || mimeType.includes("document")) {
      fileType = "document";
    }
    ...
    if (response.status === 200 || response.data?.success) {
      const successMessage = optionLabel === "Document" ? "Document sent successfully!" : "Photo/Video sent successfully!";
    ...
    const errorMsg = error.response?.data?.message || error.message || "Failed to send file";
...
const handleInvalidSession = useCallback(() => {
  ...
  const sortedCaseDetails = [...caseDetails].sort((a, b) => a.timestamp - b.timestamp);
  ...
  const handleSendTemplates = (item) => {
              ...
              {loading ? (
                ...
                <div className="flex">
                  <div className="chat-list-panel">
                    <div className="chat-list-topbar">
                      <div className="chat-filter-tabs">
                        <button onClick={() => setOnUnread(false)} className={`filter-tab${!onRead ? " active" : ""}`}>All</button>
                        <button onClick={() => setOnUnread(true)} className={`filter-tab${onRead ? " active" : ""}`} style={{ position: "relative" }}>
                          Unread
                          {msgCountUnread > 0 && (
                            <span style={{ position: "absolute", top: "-6px", right: "-6px", background: "#ef4444", color: "#fff", fontSize: "9px", fontWeight: 700, borderRadius: "9999px", minWidth: "16px", height: "16px", display: "flex", alignItems: "center", justifyContent: "center", padding: "0 3px" }}>
                      ...
                      <div className="chat-search-row">
                        <div className="chat-search-box">
                          <FaSearch style={{ color: "#9ca3af", flexShrink: 0, fontSize: "13px" }} />
                          <input type="text" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Search name or number…" />
                          {searchInput && (
                            <button onClick={() => setSearchInput("")} style={{ display: "flex", color: "#6b7280" }}><RxCross2 size={14} /></button>
                          )}
                        </div>
                        <div className="icon-btn" onClick={() => setPlusModel(true)}><CiCirclePlus size={22} /></div>
                        <div className="icon-btn" style={{ position: "relative" }} onClick={() => setFilterModel((prev) => !prev)}>
                          <IoFilterCircleOutline size={22} />
                          {(repliedChatChecked || lessThan24Checked || tagsChecked) && (
                            <span style={{ position: "absolute", top: "4px", right: "4px", width: "8px", height: "8px", borderRadius: "50%", background: "#25d366", border: "1.5px solid #fff" }} />
                          )}
                        </div>
                      </div>
                      <div className={`transition-all duration-200 overflow-hidden ${filterModel ? "max-h-[200px] opacity-100" : "max-h-0 opacity-0"}`}>
                        <div className="filter-dropdown" style={{ marginTop: "8px" }}>
                          <label><input type="checkbox" checked={repliedChatChecked} onChange={(e) => setRepliedChatChecked(e.target.checked)} /> Replied Chat</label>
                          <label><input type="checkbox" checked={lessThan24Checked} onChange={(e) => setLessThan24Checked(e.target.checked)} /> Less than 24 hours</label>
                          <label><input type="checkbox" checked={tagsChecked} onChange={(e) => setTagsChecked(e.target.checked)} /> Has Tags</label>
                        </div>
                      </div>
                    </div>
                    ...
                    <div onScroll={handleScroll} style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none" }}>
                      {filteredData
                        .slice()
                        ...
                        .map((item) => (
                          <div
                            ...
                            onClick={() => { handleGetCaseNo(item); setProfileModal(false); }}
                          >
                            <div className="chat-avatar" style={{ background: avatarGradientForName(item?.Name || item?.profile || item?.From) }}>
                              {renderAvatarContent(item?.Name || item?.profile || item?.From)}
                            </div>

                            <div className="chat-item-content">
                              <div className="chat-item-header">
                                <span className="chat-item-name">{item?.Name || item?.profile || item?.From}</span>
                                <span className="chat-item-time">
                                  {item.lastTimeStamp ? formatTimestamp(item.lastTimeStamp) : ""}
                                </span>
                              </div>
                              ...
                              <div className="chat-item-footer">
                                ...
                                >
                                  {(() => {
                                    ...
                                    if (lastMessage.interactiveMsg) {
                                      if (lastMessage.interactiveMsg.type === "flow")
                                        return "📋 Flow message";

                                      if (lastMessage.interactiveMsg.type === "button")
                                        return "🔘 Button";

                                      if (lastMessage.interactiveMsg.type === "list")
                                        return "📋 List";
                                    }
                                ...
                                {(!item?.adminReadLast) && item?.msgCount?.length > 0 && (
                                  <span className="unread-badge">{item.msgCount.length}</span>
                                )}
                              </div>
                            </div>
                          </div>
                      ...
                      {loadingChats && (
                        <div className="">
                          {Array.from({ length: (filteredData.length) > 0 ? (filteredData.length < 15 ? (15-filteredData.length) : 6) : 15}).map((_, index) => (
                            <div
                              key={index}
                      ...
                      {(filteredData.length === 0 && !loadingChats && onRead && msgCountUnread === 0) && (
                        <div className="flex flex-col items-center justify-center h-[500px] text-center p-6">
                          ...
                          <p className="text-sm text-gray-500 max-w-xs">
                            You're all caught up! When you receive new messages, they'll show up here.
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="sm:w-[96dvw] sm:h-[93dvh] flex flex-col whatsapp-chat-bg">
                    {fromNo && (
                      <div className="chat-header" onClick={() => handleGetProfileData(fromNo)} style={{ cursor: "pointer" }}>
                          ...
                          <div style={{ flex: 1 }}>
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginLeft: "auto" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: "4px", background: "#dfdfdf", padding: "4px 8px", borderRadius: "20px" ,marginRight:"12px",marginTop:"14px"}}>
                                  <input
                                    type="text"
                                  ...
                                  {matchedMessages.length > 0 && (
                                    <span style={{ fontSize: "11px", color: "#666", whiteSpace: "nowrap" }}>
                                      {currentMatch + 1}/{matchedMessages.length}
                                    </span>
                                  )}
                                  <button 
                            ...
                            <div className="chat-header-meta">
                              ...
                              {CompanyName && <><span style={{ margin: "0 5px" }}>•</span><span>{CompanyName}</span></>}
                              {Designation && <><span style={{ margin: "0 5px" }}>•</span><span>{Designation}</span></>}
                            </div>
                          </div>
                        </div>
                    ...
                    >
                      ...
                      <div className="flex flex-col w-full p-2">
                        {!loadingMessages && !fromNo && <BusinessWhatsAppPlaceholder />}
                        {loadingMessages ? (
                          ...
                          sortedCaseDetails.map((msg, index) => {
                            ...
                            const currentDateObj = new Date(msg.timestamp * 1000);
                            const prevDateObj = index > 0 ? new Date(sortedCaseDetails[index - 1].timestamp * 1000) : null;

                            const isOldest = index === 0;
                            const showHeader = !prevDateObj || !isSameDay(currentDateObj, prevDateObj);
                              ...
                              <React.Fragment key={`${msg.id}-${msg.timestamp}-${index}`}>
                                ...
                                <div ref={(el) => (messageRefs.current[msg.id] = el)}>
                                  <MessageItem
                                    ...
                                    lastTime={lastTime}
                                    userId={effectiveUserId}
                                    isLastMessage={index === caseDetails.length - 1}
                            ...
                            {replyMessage && (
                              <div className="reply-preview">
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2px" }}>
                                  <span style={{ fontSize: "11px", fontWeight: 600, color: "#075e54" }}>{Name || selectedCase?.Name}</span>
                                  <RxCross2 size={14} onClick={() => { setReplyMessage(""); setReplyId(""); }} style={{ cursor: "pointer", color: "#6b7280" }} />
                                </div>
                                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                                  <IoReturnDownForwardOutline size={13} style={{ color: "#6b7280", flexShrink: 0 }} />
                                  <span className="reply-preview-text">{replyMessage.length > 230 ? replyMessage.slice(0, 230) + "..." : replyMessage}</span>
                                </div>
                              </div>
                            )}
                            <div className={`msg-input-box${replyMessage ? " has-reply" : ""}`} style={{ position: "relative" }}>
                              {timer > 0 && (
                                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px", padding: "4px" }}>
                                  <button type="button" onClick={() => setShowPicker((p) => !p)} style={{ color: "#6b7280", display: "flex" }}>
                                    <MdOutlineEmojiEmotions size={20} />
                                  </button>
                                  <FiPlus size={18} onClick={() => setShowAttachmentModal((p) => !p)} style={{ cursor: "pointer", color: "#6b7280" }} />
                                </div>
                              )}
                              <textarea
                                value={messageValue}
                              ...
                              {showAttachmentModal && (
                                <div ref={attachmentRef} className="absolute bottom-[5.2rem] left-0 w-auto bg-white rounded-2xl border border-gray-200 z-50">
                                  <div className="grid grid-rows-2 p-3 gap-2">
                                    {attachmentOptions.map((option, index) => (
                                      <div key={index} className="flex flex-row items-center gap-2 p-2 hover:bg-gray-100 rounded-lg cursor-pointer"
                                        onClick={() => { setShowAttachmentModal(false); handleFileSelect(option.accept, option.label); }}>
                                        {option.icon}
                              ...
                              <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                                <div className="bg-gray-100 rounded-lg sm:w-fit sm:h-[85vh] overflow-hidden">
                                  <div className="flex justify-end items-center p-2">
                                    <button onClick={() => { setShowPreviewModal(false); setSelectedFile(null); setPreviewUrl(""); }} className="text-gray-500 hover:text-gray-700">
                                      <MdClose size={24} />
                                    </button>
                                  </div>
                                  <div className="p-4 flex justify-center items-center h-[70vh]">
                                    {selectedFile?.type.startsWith("image/") ? (
                                      <img src={previewUrl} alt="Preview" className="max-w-full max-h-full object-contain border" />
                                    ) : selectedFile?.type.startsWith("video/") ? (
                                      <video controls className="max-w-full max-h-full"><source src={previewUrl} type={selectedFile.type} /></video>
                                    ) : (
                                      <div className="text-center"><IoDocumentText size={48} className="mx-auto text-gray-400" /><p className="mt-2">{selectedFile?.name}</p></div>
                                    )}
                                  </div>
                                  <div className="flex justify-end items-center p-3">
                                    <button onClick={handleSendMedia}
                                      ...
                                      {sendingFile ? (
                                        <>
                                          <div style={{ width: "15px", height: "15px", border: "2px solid white", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
                                          Sending...
                                        </>
                                      ) : (
                                        <>
                          ...
                          >
                            <div className="flex justify-between items-center gap-2 px-3 py-2 bg-gray-100">
                              <div className="flex justify-start items-center gap-2">
                                ...
                                {profileDetails?.Name || profileDetails?.profile || profileDetails?.From || "-"}
                              </div>
                                ...
                                <CiEdit
                                  ...
                                  onClick={() => {
                                    ...
                                    setEditForm({
                                      ...
                                      CompanyEmail: profileDetails.CompanyEmail || "",
                                      PersonalEmail: profileDetails.PersonalEmail || "",
                                      Designation: profileDetails.Designation || "",
                                      newTag: "",
                                      GroupName: Array.isArray(profileDetails.GroupName)
                            ...
                            <div className="grid grid-cols-2 gap-y-6 gap-x-8 text-gray-700 px-4 py-3">
                              <div>
                                ...
                                <div className="text-base font-medium">
                                  {profileDetails?.Name || profileDetails?.profile || "-"}
                                </div>
                              </div>
                              <div>
